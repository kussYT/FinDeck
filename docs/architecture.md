...
**[DÉCIDÉ — FINDECK]** Le socle, les calculs et la persistance n'utilisent pas de génération de code. Les modèles sont des classes écrites à la main. `PortfolioPurchaseRepository` et `FavoriteRepository` existent pour les données utilisateur. Ils ne choisissent pas encore entre une API et un cache : cette source distante n'existe pas. Les Widgets et le domaine n'exécutent pas de SQL.
